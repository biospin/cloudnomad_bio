# 클라우드   바이오 : 클라우드 서비스를 골라서 활용하는  바이오인포메틱스. 클라우드 유목민이 되자.~~


<img src="https://github.com/biospin/cloudnomad_bio/blob/master/main02.jpg"  width="600"  height="500"/>


## 스터디 정보
저희 모임은 강의식이 아닌 자신이 스스로 공부한 내용을 여러 사람과 공유하는 스터디 방식이며, 스스로 공부하고자 하는 열정을 꼭 준비물로 가지고 와 주시고, 열심히 발표를 해주시면 더욱 감사합니다.

## 후원사
KossLab : http://devlab.oss.kr/

## 공유폴더 
https://drive.google.com/drive/u/0/folders/0B6bSLTlVnagfbjBEVFNyb2d2dms

## 교재 정보
1. 클라우드 서비스
   - 아마존웹서비스 : https://aws.amazon.com/ko/start-now/
   - MS 애저 : https://azure.microsoft.com/ko-kr/training/free-online-courses/
   - 구글 클라우드 : https://cloud.google.com/compute/docs/how-to

2. 도커 
   - 가장 빨리 만나는 Docker : http://pyrasis.com/book/DockerForTheReallyImpatient/
   - 바이오 도커 실습
       - http://biocontainers.pro/docs/101/intro/
       - https://github.com/LabAdvComp/rna-seq-docker
       - GUI docker :  https://github.com/BioDepot/GUIdock-VNC

3. 베이지안과 강화학습
   - 베이즈통계학 입문 : http://www.kyobobook.co.kr/product/detailViewKor.laf?mallGb=KOR&ejkGb=KOR&linkClass=130321&barcode=9788965022718&orderClick= 
   - 베이지안 통계 :  방통대 교재 ( 발표자에게 PPT 제공 )
   - 모두를 위한 RL강좌 : https://www.youtube.com/watch?v=dZ4vw6v3LcA&list=PLlMkM4tgfjnKsCWav-Z2F-MMFRx-2gMGG&index=1

4. 논문읽기 모임
   - PR12 딥러닝  : https://www.youtube.com/playlist?list=PLlMkM4tgfjnJhhd4wn5aj8fVTYJwIpWkS
   - 딥바이오 논문 : https://github.com/gokceneraslan/awesome-deepbio
   

## 장소
1. 장소: 토즈 신촌 아트리온점 ( http://www.toz.co.kr/branch/main/index.htm?id=6 )
2. 매주 화요일, 저녁 7시 30분~10시
3. 시작: 2017년09월 05일

## 이전 스터디 자료 
- 모두의 바이오 : https://github.com/biospin/all_bio
- 알바이오플러스 : https://github.com/biospin/R_Bio_Plus 
- 알바이오 : https://github.com/biospin/R_Bio
- 빅바이오 : https://github.com/biospin/BigBio 
- 딥바이오  : https://github.com/biospin/DeepBio


## Schedule

### Part 1.
|seq.|    날짜        |     내용                                                                                      
|----| ---------|----------------------------------------------
|  1 |2017. 9. 5|(클라우드 서비스)  아마존웹서비스  : 10분 자습서(  Linux VM 시작, 파일 저장및 검색)
|    |          |(도커 )  가장 빨리 만나는 Docker :  1장 ~ 2장
|    |          |(베이지안) 베이즈통계학 입문 : 제1강 ~ 제2강 베이즈 추정은 때로 직감에 크게 반한다
|  2 |2017. 9.12|(클라우드 서비스)  MS 애저  : "Microsoft Azure에서의 인프라 관리 - 시작" 과 "Microsoft Azure Virtual Machines 시작"
|    |          |(도커 )  가장 빨리 만나는 Docker :  3장  ~ 4장 : Docker 이미지 생성하기
|    |          |(베이지안)  베이지안 통계 : 1장. 결정이론의 틀 (1)
|  3 |2017. 9.19|(클라우드 서비스)  구글 클라우드  : "Creating and Starting an Instance" 와 "Connecting to Linux Instances"
|    |          |(도커 )  가장 빨리 만나는 Docker :  5장  ~ 6장 : Docker 좀더 활용하기
|    |          |(베이지안) 베이즈통계학 입문 : 제3강 ~ 제4강 ‘확률의 확률’을 사용하여 추정의 폭을 넓힌다  
|  4 |2017. 9.26|(도커 )  가장 빨리 만나는 Docker :  7장  ~ 8장 : Docker로 애플리케이션 배포하기
|    |          |(베이지안)  베이지안 통계 : 2장.결정이론의 틀 (2)
|    |          |(논문읽기 모임)  관심 있는 논문 읽기

### Part 2.
|seq.|    날짜        |     내용                                                                                      
|----| ---------|----------------------------------------------
|  1 |2017.10.10|(도커 )  가장 빨리 만나는 Docker :  7장  ~ 8장 : Docker로 애플리케이션 배포하기
|    |          |(베이지안) 베이즈통계학 입문 : 제5강 ~ 제6강 명쾌하고 엄밀하지만 쓸 데가 한정된 
|    |          |(베이지안)  모두를 위한 RL강좌 : 1장 ~ 2장 : Playing OpenAI GYM Games
|  2 |2017.10.17|(도커 )  가장 빨리 만나는 Docker :  9장  ~ 10장 : Amazon Web Services에서 Docker 사용하기
|    |          |(베이지안)  베이지안 통계 : 3장.베이지안 추론 I : 사후분포
|    |          |(논문읽기 모임)  관심 있는 논문 읽기
|  3 |2017.10.24|(도커 )  가장 빨리 만나는 Docker :  11장  ~ 12장 : Microsoft Azure에서 Docker 사용하기
|    |          |(베이지안) 베이즈통계학 입문 : 7강 ~ 제8강 베이즈 추정은 ‘최우원리’에 근거해 있다 
|    |          |(베이지안)  모두를 위한 RL강좌 : 3장 : Dummy Q-learning (table)
|  4 |2017.10.31|(도커 )  가장 빨리 만나는 Docker :  13장  ~ 14장 : Docker Remote API 사용하기
|    |          |(베이지안)  베이지안 통계 : 4장. 베이지안 추론 II : 추정과 가설검정
|    |          |(논문읽기 모임)  관심 있는 논문 읽기

### Part 3.
|seq.|    날짜        |     내용                                                                                      
|----| ---------|----------------------------------------------
|  1 |2017.11. 7|(도커 )  가장 빨리 만나는 Docker :  15장 : CoreOS 사용하기
|    |          |(베이지안) 베이즈통계학 입문 : 제9강 ~ 제10강 복수의 정보를 얻었을 때의 추정
|    |          |(베이지안)  모두를 위한 RL강좌 : 4장 : Q-learning (table) exploit&exploration and discounted 
|  2 |2017.11.14|(도커 )  가장 빨리 만나는 Docker :  16장 : Docker로 워드프레스 블로그 구축하기
|    |          |(베이지안)  베이지안 통계 : 5장. R의 소개와 이항모형 실습
|    |          |(논문읽기 모임)  관심 있는 논문 읽기
|  3 |2017.11.21|(도커 )  가장 빨리 만나는 Docker :  19장 : Docker 활용 시나리오
|    |          |(베이지안) 베이즈통계학 입문 : 11강 ~ 제12강 베이즈 추정에서는 정보를 순차적으로 사용할 수 있다 
|    |          |(베이지안)  모두를 위한 RL강좌 : 5장 : Q-learning on Nondeterministic Worlds!
|  4 |2017.11.28|(도커 )  바이오 도커 실습  :  http://biocontainers.pro/docs/101/intro/  에서  Basic
|    |          |(베이지안)  베이지안 통계 : 6장. 베이지안 추론 III : 정규모형
|    |          |(논문읽기 모임)  관심 있는 논문 읽기 

### Part 4.
|seq.|    날짜        |     내용                                                                                      
|----| ---------|----------------------------------------------
|  1 |2017.12. 5|(도커 )  바이오 도커 실습  :  http://biocontainers.pro/docs/101/intro/  에서  Developer tutorial과 Containers Examples
|    |          |(베이지안) 베이즈통계학 입문 : 제13강 ~ 제14강 ‘확률’은 ‘면적’과 동일한 성질을 지닌다
|    |          |(베이지안)  모두를 위한 RL강좌 : 6장 : Q-Network
|  2 |2017.12.12|(도커 )  바이오 도커 실습  :  https://github.com/LabAdvComp/rna-seq-docker  : 다른 실습으로 대체 가능함
|    |          |(베이지안)  베이지안 통계 : 8장. 몬테카를로 방법
|    |          |(논문읽기 모임)  관심 있는 논문 읽기
|  3 |2017.12.19|(도커 )  바이오 도커 실습  :  GUI docker :  https://github.com/BioDepot/GUIdock-VNC   : 다른 실습으로 대체 가능함
|    |          |(베이지안) 베이즈통계학 입문 :  15강 ~ 16강 더 범용적인 추정을 위한 ‘확률분포도’ 
|    |          |(베이지안)  모두를 위한 RL강좌 : 7장 : DQN
|  4 |2018. 1. 2|(도커 )  바이오 도커 실습  :  실습 ???????????
|    |          |(베이지안)  베이지안 통계 : 9장. 깁스샘플링
|    |          |(논문읽기 모임)  관심 있는 논문 읽기 

### Part 5.
|seq.|    날짜        |     내용                                                                                      
|----| ---------|----------------------------------------------
|  1 |2018. 1. 9|(도커 )  바이오 도커 실습  :  실습 ???????????
|    |          |(베이지안) 베이즈통계학 입문 : 제 17강 ~ 18강 확률분포의 성격을 결정짓는 ‘기대치’ 
|    |          |(베이지안)  베이지안 통계 : 9장. 깁스샘플링
|  2 |2018. 1.16|(도커 )  바이오 도커 실습  :  실습 ???????????
|    |          |(베이지안) 베이즈통계학 입문 : 제9강 베이즈 추정은 때로 직감에 크게 반한다
|    |          |(베이지안)  베이지안 통계 : 10장. 선형모형
|  3 |2018. 1.23|(도커 )  바이오 도커 실습  :  실습 ???????????
|    |          |(베이지안) 베이즈통계학 입문 : 제19강 확률분포도를 사용한 고도의 추정①
|    |          |(베이지안)  베이지안 통계 : 11장. 일반화 선형회귀모형과 R 실습
|  4 |2018. 1.30|(도커 )  바이오 도커 실습  :  실습 ???????????
|    |          |(베이지안) 베이즈통계학 입문 : 제20강 동전 던지기나 천체 관측에서 관찰되는 
|    |          |(베이지안)  베이지안 통계 : 12장. 시계열 잔차 선형회귀모형과 R실습
|  5 |2018. 2. 6|(도커 )  바이오 도커 실습  :  실습 ???????????
|    |          |(베이지안) 베이즈통계학 입문 : 제21강 확률분포도를 사용한 고도의 추정② 
|    |          |(베이지안)  베이지안 통계 : 13장. 혼합효과 선형회귀모형과 R실습





